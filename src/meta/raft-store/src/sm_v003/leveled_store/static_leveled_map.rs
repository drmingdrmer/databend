...
use crate::sm_v003::leveled_store::util;
...
impl StaticLeveledMap {
    pub(in crate::sm_v003) fn new(levels: impl IntoIterator<Item = Arc<LevelData>>) -> Self {
        Self {
    ...
    pub(in crate::sm_v003) fn iter_levels(&self) -> impl Iterator<Item = &LevelData> {
        self.levels.iter().map(|x| x.as_ref()).rev()
    }

    pub(in crate::sm_v003) fn newest(&self) -> Option<&Arc<LevelData>> {
        self.levels.last()
    }

    pub(in crate::sm_v003) fn push(&mut self, level: Arc<LevelData>) {
        self.levels.push(level);
    }

    pub(in crate::sm_v003) fn len(&self) -> usize {
        self.levels.len()
    }
}
